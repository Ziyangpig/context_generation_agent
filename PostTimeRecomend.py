...
def preprocess(df):
    ...
    # 构造特征矩阵X
    features = ['hour', 'weekday_0','weekday_1','weekday_2','weekday_3','weekday_4','weekday_5','weekday_6', 'scaled_score']
    X = high_interact_df[features]
    return X, high_interact_df

# ==============================
# 使用K-means进行聚类
# ==============================

def auto_select_k(X, max_k=10):
    """
    自动选择最佳K值
    :param X: 标准化后的特征矩阵
    :param max_k: 最大尝试K值
    ...
    return metrics,k_counts.most_common(1)[0][0]
...
def post_time_recom_main():
    df = fake_data()
    X, high_interact_df = preprocess(df)
    #TODO: 动态更新 保留半年数据，最新时间传递给爬虫开始爬取时间 
    
    # 投票机制选择K值
    metrecs, optimal_k = auto_select_k(X, max_k=10)
    print(f"投票机制选择K值:\n{metrecs}")
    ...
    high_interact_df['cluster'] = kmeans.fit_predict(X)
    
    cluster_summary = res_info(high_interact_df)
